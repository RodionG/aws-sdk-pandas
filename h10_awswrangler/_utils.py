...
from h10_awswrangler import _config, exceptions
from h10_awswrangler.__metadata__ import __version__
...
def default_botocore_config() -> botocore.config.Config:
    ...
    return botocore.config.Config(
        ...
        user_agent_extra=f"h10_awswrangler/{__version__}",
    )
...
def parse_path(path: str) -> Tuple[str, str]:
    ...
    Examples
    --------
    >>> from h10_awswrangler._utils import parse_path
...
def ensure_cpu_count(use_threads: Union[bool, int] = True) -> int:
    ...
    Examples
    --------
    >>> from h10_awswrangler._utils import ensure_cpu_count
...
def chunkify(lst: List[Any], num_chunks: int = 1, max_length: Optional[int] = None) -> List[List[Any]]:
    ...
    Examples
    --------
    >>> from h10_awswrangler._utils import chunkify
